...
class Cursor:
    ...
    def event_controller(self, event, doors, scroll):
        ...
        collide_door = None
        for door in doors:
            if door_collision((self.rect[0], self.rect[1], self.rect[2], self.rect[3]), door.line_collider.line_collider):
                collide_door = door
                break
    ...
    def paint(self, x, y, sc):
        ...
        # pygame.draw.rect(sc, 'red', (self.rect[0] - self.player.scroll[0], self.rect[1] - self.player.scroll[1], self.rect.w, self.rect.h))
